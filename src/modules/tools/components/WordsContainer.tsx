...
export const WordsContainer = ({ message, list, onDelete, onClear }: Props) => {
  ...
  return (
    <div className="relative">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              onClick={() => onClear}
            >
              <XCircle className="text-foreground/50" />
            </Button>
          </TooltipTrigger>
      ...
      <div className="w-full h-32 overflow-y-auto border rounded-lg bg-accent/20 flex gap-2 items-start justify-start p-4">
        {list.length === 0 && (
        ...
        {list.map((word, i) => (
          <Badge key={word + i} variant={"secondary"} className="text-base">
            {word}{" "}
            <span
