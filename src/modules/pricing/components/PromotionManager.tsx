  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-foreground">Promotions and Offers</h2>
          <p className="text-sm text-muted-foreground">Manage discounts, offers, and scheduled promotions</p>
        </div>
        <Button
          onClick={() => {
            setEditingPromotion(undefined)
      ...
      <CustomEmpty description="Create your first promotion to get started" icon={Percent} title="No Promotions Yet" onClick={() =>{
        setEditingPromotion(undefined);
        setDialogOpen(true)
      }} />
      ) : (
        <div className="grid gap-4">
          {promotions.map((promotion) => {
            ...
            return (
              <Card key={promotion.id} className={active ? "border-success" : ""}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1 space-y-1">
                      ...
                      <CardDescription className="flex items-center gap-4">
                        <span className="font-mono font-semibold text-foreground">{promotion.code}</span>
                        <span className="flex items-center gap-1">
                          <Percent className="h-3 w-3" />
                          {promotion.discount}
                          {promotion.discountType === "percentage" ? "%" : " USD"} discount
                        </span>
                      </CardDescription>
                    </div>
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                    <div className="flex items-center gap-2">
                      ...
                      <span>From: {format(promotion.startDate, "MMM dd, yyyy")}</span>
                    </div>
                    {promotion.endDate && (
                      <div className="flex items-center gap-2">
                        <Clock className="h-4 w-4" />
                        <span>To: {format(promotion.endDate, "MMM dd, yyyy")}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2">
