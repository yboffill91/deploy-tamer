    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Configure available plans for your tools</p>
        </div>
        <Button
          onClick={() => {
            setEditingPlan(undefined)
      ...
      <CustomEmpty description="Create yout first pricing to get started" icon={Package} title="No Plans Yet" 
      onClick = {() => {
        setEditingPlan(undefined)
        setDialogOpen(true)
      }}
      ...
      />
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredPlans.map((plan) => {
            ...
            return (
              <Card key={plan.id} className={cn('', plan.recommended && 'bg-success/5 border-success bprder-2')}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      ...
                      <CardDescription>
                        ...
                        <span className="text-muted-foreground">/{plan.interval === "month" ? "mo" : "yr"}</span>
                      </CardDescription>
                    </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  {planRequirements.length > 0 && (
                    <div className="border-t pt-4">
                      <h4 className="text-sm font-medium mb-3">Requirements ({planRequirements.length})</h4>
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="h-9 text-xs text-center w-16">Req.</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {planRequirements.map((req) => (
                              <TableRow key={req.id}>
                                <TableCell className="py-2 text-xs font-medium">{req.name}</TableCell>
                                <TableCell className="py-2">
                                  <Badge variant="outline" className={`text-xs ${getCategoryColor(req.category)}`}>
      ...
      <PlanDialog open={dialogOpen} onOpenChange={setDialogOpen} plan={editingPlan} onSave={handleSavePlan} />
    </div>
  )
}
